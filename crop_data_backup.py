...
# --- UGOS/VGOS/SLA/SSH (tout dans un seul fichier) ---
print("UGOS/VGOS/SLA/SSH...")
ds_uv = xr.open_dataset(
    "/Odyssey/private/d21botvy/FORECAST/2023a_SSH_mapping_OSE/ssh_2023_Pierre_GLO12/2023a_SSH_mapping_OSE/nb_diags_global/UNet_sla_real_data_training_Nadir_filt_DUACS_losses_2017-2022_all_days_GEOS_velocities_leadtime_0.nc",
    chunks={"time": 50},
)
ds_uv = ds_uv.rename({"latitude": "lat", "longitude": "lon"})
ds_uv = ds_uv.sel(lon=slice(LON_MIN, LON_MAX), lat=slice(LAT_MIN, LAT_MAX))
ds_uv = ds_uv.interp(lon=target_lon, lat=target_lat, method="linear").load()

# --- SST ---
print("SST...")
ds_sst = xr.open_dataset(
    "/Odyssey/public/glorys/rec/glorys4_global_1patch_SST_ODYSSEA_UNet_L3_NRT_AnomalyCLIMATO_2010_2019_NormStatsImposed_MSEGradLoss_pyresample_0.25degL3_INPUT_INFRARED_FLAGGED_5_SLA_grid_SLA_INPUT_src_CORRECTED/nrt_sla/ABSOLUTE_SST/test_data_14.nc",
    chunks={"time": 50},
)
...
ds_sst = ds_sst.interp(lon=target_lon, lat=target_lat, method="linear").load()
...
# --- MERGE ---
print("Merge...")
ds = xr.merge([ds_uv, ds_sst, ds_mdt, ds_bathy, ds_sss], compat="override", join="outer")
