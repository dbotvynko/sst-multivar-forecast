...
from contrib.ose_pipeline.ose_rec_pipeline import execute_rec_pipeline, execute_rec_pipeline_L4, execute_rec_pipeline_SST_SLA_INOUT, execute_rec_pipeline_SST_SLA_INOUT_with_inputs, execute_rec_pipeline_SST_SLA_WIND_INOUT
...
def execute_full_pipeline_SST_SLA_INOUT(
        pixels_per_degree,
        min_time,
        max_time,
        min_lon,
    ...
    """
    Full pipeline for SST+SLA INPUT and SST+SLA OUTPUT inference.

    ose_data_path  : path to the SST L3 NRT .nc file (used directly as
                     gridded_input — data download is skipped)
    sla_input_path : path to the gridded along-track SLA .nc file
                     (variable 'sla_unfiltered' or first variable)
    tgt_sla_path   : path to the reference SLA .nc file for evaluation
                     (variable 'zos', 'sla', or first variable)
    """
    (
        dl_sat_input_dir,
        dl_sat_ref_dir,
    ...
    if not skip['rec']:
        execute_rec_pipeline_SST_SLA_INOUT(
            model_config_path=model_config_path,
            model_ckpt_path=model_ckpt_path,
            rec_path=rec_path,
            rec_paths=rec_paths,
...
def execute_full_pipeline_SST_SLA_INOUT_with_inputs(
        pixels_per_degree,
        min_time,
        max_time,
        min_lon,
    ...
    """
    Same as execute_full_pipeline_SST_SLA_INOUT but saves from j-1
    and also saves raw input observations alongside model outputs.
    """
    (
        dl_sat_input_dir,
        dl_sat_ref_dir,
    ...
    if not skip['rec']:
        execute_rec_pipeline_SST_SLA_INOUT_with_inputs(
            model_config_path=model_config_path,
            model_ckpt_path=model_ckpt_path,
            rec_path=rec_path,
            rec_paths=rec_paths,
    ...
    if not skip['metrics']:
        print('-' * 60 + '\nMETRICS PIPELINE SKIPPED (not applicable for with_inputs)\n' + '-' * 60)
    else:
        print('-' * 60 + '\nMETRICS PIPELINE SKIPPED\n' + '-' * 60)


def execute_full_pipeline_SST_SLA_WIND_INOUT(
        pixels_per_degree,
        min_time,
        max_time,
        min_lon,
    ...
    """
    Full pipeline for SST+SLA+Wind INPUT → SST+SLA OUTPUT inference.
    """
    (
        dl_sat_input_dir,
        dl_sat_ref_dir,
    ...
    if not skip['rec']:
        execute_rec_pipeline_SST_SLA_WIND_INOUT(
            model_config_path=model_config_path,
            model_ckpt_path=model_ckpt_path,
            rec_path=rec_path,
            rec_paths=rec_paths,
    ...
    if not skip['metrics']:
        print('-' * 60 + '\nMETRICS PIPELINE SKIPPED (wind model)\n' + '-' * 60)
    else:
        print('-' * 60 + '\nMETRICS PIPELINE SKIPPED\n' + '-' * 60)
